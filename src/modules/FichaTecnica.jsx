...
export default function FichaTecnica({ onNav }) {
  ...
  return (
    <div className="p-4 md:p-8 space-y-5 md:space-y-6 max-w-7xl">
      <PageHeader title="Ficha Tecnica">
        ...
        {receitas.map(prato => {
          ...
          return (
            <div key={prato.id} className="bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 shadow-sm overflow-hidden">
              <div
                className="px-4 md:px-5 py-3 md:py-4 cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors"
                onClick={() => setExpanded(isOpen ? null : prato.id)}
              >
                <div className="flex items-center gap-3">
                  <span className="text-gray-400 dark:text-gray-500 text-sm w-4 shrink-0">{isOpen ? '▼' : '▶'}</span>
                  <div className="flex-1 min-w-0">
                    <div className="font-bold text-gray-900 dark:text-white text-sm truncate">{prato.nome}</div>
                    <div className="text-xs text-gray-400 dark:text-gray-500 mt-0.5">{prato.cat}</div>
                  </div>

                  {/* Mobile: so a margem no header */}
                  <div className="md:hidden text-right shrink-0">
                    <div className="text-[10px] uppercase tracking-wider text-gray-400">Margem</div>
                    <div className={`font-bold text-sm ${margem >= 0.5 ? 'text-green-600 dark:text-green-400' : 'text-primary'}`}>
                      {fmtP(margem)}
                    </div>
                  </div>

                  {/* Desktop: metricas + acoes */}
                  ...
                  <div className="hidden md:flex gap-2 shrink-0" onClick={e => e.stopPropagation()}>
                ...
                {/* Mobile: linha de detalhes + acoes */}
                <div className="md:hidden mt-2 ml-7 flex items-center justify-between gap-2">
                  <div className="flex gap-4 text-xs text-gray-500 dark:text-gray-400">
                    <span>Custo <strong className="text-gray-900 dark:text-white">{fmtR(custo)}</strong></span>
                    <span>Venda <strong className="text-green-600 dark:text-green-400">{fmtR(prato.preco)}</strong></span>
                  </div>
                  <div className="flex gap-2" onClick={e => e.stopPropagation()}>
                    <button onClick={() => setPratModal({ open: true, data: prato })}
                      className="px-2.5 py-1 rounded-md border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-700 text-gray-600 dark:text-gray-300 text-xs cursor-pointer">
                      Editar
                    </button>
                    <button onClick={() => setConfirm({ id: prato.id, nome: prato.nome })}
                      className="px-2.5 py-1 rounded-md border border-red-100 dark:border-red-900 bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 text-xs cursor-pointer">
                      Excluir
                    </button>
                  </div>
                </div>
